...
router.post(
  ...
  authCheck,
  asyncHandler(async (req, res) => {
    const checkAvailabiltyUser = await database('users')
      .where({ ktp: req.body.ktp })
      .first()
    ...
    const checkAvailabiltyPhone = await database('users')
    ...
    const checkAvailabiltyUserName = await database('users')
      .where({ username: req.body.username })
      .first()

    if (checkAvailabiltyUserName) {
      throw new InvariantError(
        `User name: ${req.body.username} sudah terdaftar`
      )
    }

    const checkAvailabiltyEmail = await database('users')
    ...
    if (req.files) {
      ...
      if (!allowedType.includes(ext.toLowerCase()))
        return res
          .status(422)
          .json({ msg: 'Format gambar yang di izinkan .png .jpg .jpeg' })
      if (fileSize > 5000000)
        return res
          .status(422)
          .json({ msg: 'Ukuran gambar tidak lebih dari 5 MB' })
    ...
    formData.append('picUrl', imageFileName)

    const userData = await createUser(formData)

    const userDataResponse = await database('users')
      .insert({
        ...userData,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!req.files || !req.files.image) {
      ...
      if (!allowedType.includes(ext.toLowerCase())) {
        return res
          .status(422)
          .json({ msg: 'Format gambar yang diizinkan: .png, .jpg, .jpeg' })
      }

      if (fileSize > 5000000) {
        return res
          .status(422)
          .json({ msg: 'Ukuran gambar tidak boleh lebih dari 5 MB' })
      }
